...
// The explain prompt (core/explain.js) asks for a structured, ~700-1000 word
// answer across up to eight sections - including code-quality notes drawn
// from real sampled source files - and a low cap here was cutting the
// response off mid-section (often losing the Verdict entirely, the most
// important part). 2500 gives real headroom for that depth.
...
async function callGemini(apiKey, systemPrompt, userPrompt) {
    ...
    try {
        ...
        // Fall back to a separate model - not just on 429 (quota exhausted)
        // but also once the primary model's own 503 retries are exhausted,
        // since that can mean a sustained outage/degradation on that
        // specific model rather than a few-second blip fetchWithRetry
        // already covers. Live-verified: a persistent 503 on
        // gemini-flash-latest kept failing through its own retries with no
        // sign of clearing.
    ...
    if (!res.ok) {
        ...
        const message = modelUsed === GEMINI_FALLBACK_MODEL
            ? `Gemini is unavailable on both the primary and fallback models right now (HTTP ${res.status}) - this looks like a broader outage or quota exhaustion, not something a quick retry fixes. Wait a while and try again, or add your own AI API key in Settings.`
